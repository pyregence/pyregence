...
(comment
  ...
  (->>
    ...
    :params
    :pattern)
  ;; => {:default-option :UtilityCompany,
  ;;     :options {:utilitycompany {:auto-zoom? true,
  ;;                              :disabled-for #{:crown-fire-area},
  ;;                              :filter "utility company",
  ;;                              :geoserver-key :psps,
  ;;                              :opt-label "Utility Energy lines",
  ;;                              :selected? true}}}




;; the layer-config is invalid
  (s/valid? ::layer-config {:default-option :UtilityCompany})
  ;; => false

  ;; the layer path is fine
  (s/valid? ::layer-path [:fire-risk :params :pattern])
  ;; => true

  ;; the layer config is invalid because

  (s/explain-str ::layer-config {:default-option :UtilityCompany})
  ;; => "{:default-option :UtilityCompany} - failed: (contains? % :opt-label) spec: :pyregence.pages.near-term-forecast/layer-config\n{:default-option :UtilityCompany} - failed: (or (contains? % :filter) (contains? % :filter-set)) spec: :pyregence.pages.near-term-forecast/layer-config\n"

  (s/explain-str ::layer-config {:default-option :UtilityCompany :opt-label ""})
  ;; => "{:default-option :UtilityCompany, :opt-label \"\"} - failed: (or (contains? % :filter) (contains? % :filter-set)) spec: :pyregence.pages.near-term-forecast/layer-config\n"

  (s/explain-str ::layer-config {:default-option :UtilityCompany :opt-label "" :filter ""})
  ;; => "Success!\n"

  ;; so one option is to add the opt-label and filter with meaningless params
  ;; another option is to make them optional
  ;; a third is to remove the spec validation and have it come when inserting data into the database.

  ;; is any of the data currently invalid?

  (reduce (fn [invalid-layer-pair {:keys [layer_path layer_config]}]
            (let [layer-path   (edn/read-string layer_path)
                  layer-config (edn/read-string layer_config)]
              (if-not (and (s/valid? ::layer-path   layer-path)
                           (s/valid? ::layer-config layer-config))
                (conj [layer-path layer-config])
                invalid-layer-pair)))
          []
          user-layers)
  ;; => [[:fire-risk :params :pattern] {:default-option :utilitycompany}]
  ...
  ;; there is no difference between assoc and update right?
  ...
  (let [[things-only-in-update things-only-in-assoc things-in-both] (clojure.data/diff update-v assoc-v)]
       things-only-in-update)
  ;; => summary, update-v has the correct values because as predicated assoced values get overwritten.
