            ...
            (case selected-page
              ...
              [os/main
               (let [;;TODO this selection should probably be resolved earlier on or happen a different way aka not create org-id->org if only one org
                     ...
                     roles (->> user-role roles/role->roles-below (filter (fn [role] (roles/organization-roles role))))
                     selected-orgs-users
              ...
              (let [roles (->> user-role roles/role->roles-below (filter (fn [role] (roles/none-organization-roles role))))]
