...
^:rct/test
(comment
  ;; --- The happy path returns the normalized, sanitized string ---
  (normalize-email "  Liquori.Alice@Example.COM ")
  ;=> "liquori.alice@example.com"

  ;; Transforms feed checks: surrounding whitespace is repaired before the
  ;; shape check runs, so this passes rather than failing on the spaces.
  (normalize-email "a@b.co ")
  ;=> "a@b.co"
  ...
  ;; --- Unicode-property whitelists admit names from any script... ---
  (mapv (normalizer "Name" user-name-steps)
        ["josé" "Xi Jinping" "习近平" "Björk Guðmundsdóttir" "Ngô Đình Diệm"])
  ;=> ["josé" "Xi Jinping" "习近平" "Björk Guðmundsdóttir" "Ngô Đình Diệm"]

  ;; ...normalize combining accents (e + U+0301) to precomposed NFC (U+00E9)...
  (= "Jos\u00e9" ((normalizer "Name" user-name-steps) "Jose\u0301"))
  ;=> true
  ...
  ;; --- Char classes expand into the sets and ranges they support ---
  (char-class->english #"[a-zA-Z0-9._\\-]")
  ;=> "lowercase letters (a-z), uppercase letters (A-Z), digits (0-9), and the characters . _ -"

  (char-class->english (unicode-pattern "[\\p{L}\\p{M}\\p{Nd} .,'\\-]"))
  ;=> "letters (any language), accent marks, digits (0-9), spaces, and the characters . , ' -"

  (re-find (invert-char-class #"[a-z]") "abcXdef")
  ;=> "X"

  ;; --- `report` powers live checklist UIs from the same single source ---
  (report password-steps "abc")
  ;=>> [{:valid? false} {:valid? false} {:valid? true} {:valid? false} {:valid? true}]

  ;; --- Optional fields: blank passes through as nil, present is validated ---
  (normalize-org-name nil)
  ;=> nil
  ...
  ;; ...and returns the normalized map when everything is valid.
  (validate-all! [[:email    "Email"    email-steps    " A@B.co "]
                  [:password "Password" password-steps "GoodPassw0rdHere"]])
  ;=> {:email "a@b.co" :password "GoodPassw0rdHere"}
