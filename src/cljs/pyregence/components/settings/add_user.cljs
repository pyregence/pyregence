...
(defn- add-new-users!
  [org-unique-id users]
  (go (<! (u-async/call-clj-async! "add-org-users" org-unique-id users))))
...
(defn invite-modal
  [{:keys [on-click-close-dialog default-role-option org-unique-id role-options]}]
...
(defn add-user-dialog [{:keys [org-unique-id role-options default-role-option]}]
