...
(defn- information-div [units info-height convert]
  (r/create-class
   {:component-did-mount
    ...
    :render
    (fn [_]
      (let [cleaned-last-clicked-info (u-misc/replace-no-data-nil @!/last-clicked-info
                                                             @!/no-data-quantities)
        ...
        [:div {:style {:bottom "0" :position "absolute" :width "100%"}}
         [:label {:style {:margin-top ".6rem" :text-align "center" :width "100%"}}
          (if-let [value (:band current-point)]
            (str (if (fn? convert)
                   (u-misc/round-last-clicked-info (convert value))
                   (u-misc/round-last-clicked-info value))
                 (u-misc/clean-units units))
...
(defn- single-point-info [box-height _ units convert no-convert]
  (let [legend-map  (u-misc/mapm (fn [li] [(js/parseFloat (get li "quantity")) li]) @!/legend-list)
        legend-keys (sort (keys legend-map))
        color       (or (get-in legend-map [(-> @!/last-clicked-info
                                                (max (first legend-keys))
                                                (min (last legend-keys)))
                                            "color"])
                        (let [[low high] (u-misc/find-boundary-values @!/last-clicked-info legend-keys)]
                          (when (and high low)
        ...
        *inputs     (->> @!/*params
                         ...
                         (vals)
                         (into #{}))
        add-units   #(u-str/end-with % (u-misc/clean-units units))
